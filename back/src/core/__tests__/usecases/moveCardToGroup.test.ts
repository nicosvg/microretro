...
describe("moveCardToGroup usecase", () => {
  ...
  const moveCard = moveCardToGroup(groupRepo, cardRepo, pubSub);
  ...
  test("should move a card to a new group when destination card has no group", async () => {
    ...
    // Act
    const { createdGroup: newGroupResult } = await moveCard(
      boardId,
      sourceCardId,
      destinationCardId,
    );
  ...
  test("should move a card to an existing group when destination card already has a group", async () => {
    ...
    // Act
    const { createdGroup: result, updatedSourceCard } = await moveCard(
      boardId,
      sourceCardId,
      destinationCardId,
    );
  ...
  test("should not delete source group if it still has cards after move", async () => {
    ...
    // Act
    const { createdGroup } = await moveCard(
      boardId,
      sourceCardId,
      destinationCardId,
    );
